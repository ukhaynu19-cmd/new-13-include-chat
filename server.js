...
function buildGrid(routineList) {
  ...
  const periodTimes = {};
  periods.forEach(p => {
    const match = routineList.find(r => String(r.period) === p && r.time);
    periodTimes[p] = match ? match.time : '';
  });

  return { periods, periodTimes, days: DAY_ORDER, byDayPeriod };
}
...
const storage = new CloudinaryStorage({
  ...
  params: {
    folder: 'hillacademiccare-uploads',
...
app.post('/teacher/profile/photo', requireTeacher, upload.single('photo'), async (req, res) => {
  try {
    if (req.file) {
      await Teacher.updateOne({ id: req.session.teacherId }, { photoUrl: req.file.path });
    }
...
app.post('/student/profile/photo', requireStudent, upload.single('photo'), async (req, res) => {
  try {
    if (req.file) {
  await Student.updateOne({ id: req.session.studentId }, { photoUrl: req.file.path });
}
